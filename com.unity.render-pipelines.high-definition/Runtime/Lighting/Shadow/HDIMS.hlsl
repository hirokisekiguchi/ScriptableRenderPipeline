...
float SampleShadow_IMS(float3 tcs, Texture2D shadowTexture, float depthBias, float momentBias, float shadowIntensity)
{
    // Coordinates of the shadowmap that we need to sample
    float2 shadowmapTexCoords = tcs.xy;

    // Depth value in the [-1, 1]
    float2 pixelShadowmapDepth = (1.0 - tcs.z) * 2.0f - 1.0f;

    // Sample the optimized moments texture
    float4 optimizedMomentSamples = SAMPLE_TEXTURE2D_LOD(shadowTexture, s_linear_clamp_sampler, shadowmapTexCoords, 0);

    // Revert the "optimization"
    float4 biasedMoments =  float4(0, 0, 0, 0);
    biasedMoments.xz = mul(optimizedMomentSamples.xz - 0.5f, float2x2(-1.0f / 3.0f, -0.75f, sqrt(3.0f), 0.75f * sqrt(3.0f)));
    biasedMoments.yw = mul(optimizedMomentSamples.yw, float2x2(0.125f, -0.125f, 1.0f, 1.0f));

    float finalShadowIntensity  = 1.0f;
    // Incorporate the moment bias
    biasedMoments = lerp(biasedMoments, float4(0.0f, 0.628f, 0.0f, 0.628f), momentBias);

    // Compute the shadow intensity    
    Compute4MomentUnboundedShadowIntensity(finalShadowIntensity, biasedMoments, pixelShadowmapDepth, depthBias);

    // Scale the shadow intensity
    ScaleShadowIntensity(finalShadowIntensity, finalShadowIntensity, shadowIntensity, shadowmapTexCoords, pixelShadowmapDepth);
    
    // and we are done
    return 1.0 - finalShadowIntensity;
}
*/
