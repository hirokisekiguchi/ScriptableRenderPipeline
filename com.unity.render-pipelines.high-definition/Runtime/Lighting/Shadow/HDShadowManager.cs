...
namespace UnityEngine.Experimental.Rendering.HDPipeline
{
    ...
    public class HDShadowManager : IDisposable
    {
        ...
        public HDShadowManager(RenderPipelineResources renderPipelineResources, int width, int height, int maxShadowRequests, DepthBits atlasDepthBits, Shader clearShader, bool momentShadows)
        {
            ...
            m_CascadeAtlas = new HDShadowAtlas(renderPipelineResources, 1, 1, HDShaderIDs._CascadeShadowAtlasSize, clearMaterial, momentShadows, depthBufferBits: atlasDepthBits, name: "Cascade Shadow Map Atlas");
        ...
        public void RenderShadows(ScriptableRenderContext renderContext, CommandBuffer cmd, CullingResults cullResults, HDCamera hdCamera)
        {
            ...
            // Alright, if we are in very high quality we need to compute the sum tables for the cascade shadow map
            var hdAsset = (GraphicsSettings.renderPipelineAsset as HDRenderPipelineAsset);
            if (hdAsset.renderPipelineSettings.hdShadowInitParams.shadowQuality == HDShadowQuality.VeryHigh)
            {
